...
def get_matching_dns_records(zone_id, instance_name):
    route53 = boto3.client('route53')
    records = route53.list_resource_record_sets(HostedZoneId=zone_id, MaxItems='500')
    matching_records = []
    for record in records['ResourceRecordSets']:
        if record['Type'] == 'A' and not record['Name'].startswith(instance_name) and not record.get('AliasTarget'):
    ...
    for reservation in instances:
        for instance in reservation['Instances']:
            ...
            for zone in matching_zones:
                ...
                matching_records = get_matching_dns_records(zone_id, instance_name)
